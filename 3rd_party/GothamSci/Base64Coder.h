...
#pragma once
	...
	#include <windows.h>
	#include <string>
//	typedef std::string	String;    //changed 2012-2-17 ebt for RAD Studio support

	#include <classes.hpp>
#endif

class PACKAGE Base64Coder
{
...
public:
	virtual unsigned long		Encode(const PBYTE, DWORD);
	virtual unsigned long		Decode(const PBYTE, DWORD);
	virtual unsigned long		Encode(LPCSTR sMessage);
	virtual unsigned long		Decode(LPCSTR sMessage);
