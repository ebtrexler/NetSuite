...
void NetworkVisualizationPanel::DrawSquareLattice(wxPaintDC &dc)
{
    ...
    for (const auto &pair : m_network->GetElectrodes())
    {
        ...
        if (electrode->Owner())
        {
            ...
            DrawElectrode(dc, pos.x, pos.y, electrode == m_selectedItem);
        }
    }
}
...
void NetworkVisualizationPanel::DrawCirclePerimeter(wxPaintDC &dc)
{
    ...
    for (const auto &pair : m_network->GetElectrodes())
    {
        ...
        if (electrode->Owner() && cellCount > 0)
        {
            ...
            DrawElectrode(dc, x, y, electrode == m_selectedItem);
        }
    }
}

void NetworkVisualizationPanel::DrawElectrode(wxPaintDC &dc, int x, int y, bool selected)
{
    ...
    // Draw colored needle on top (thinner)
    wxColour needleColor = selected ? *wxYELLOW : wxColour(0, 255, 0);
    dc.SetPen(wxPen(needleColor, 2));
    dc.DrawLine(startX, startY, endX, endY);
    dc.DrawLine(endX, endY, tipX1, tipY1);
    dc.DrawLine(endX, endY, tipX2, tipY2);
}
        ...
        if (cellCount > 0)
        {
            ...
            for (const auto &pair : m_network->GetElectrodes())
            {
                ...
                if (electrode->Owner())
                {
                    ...
                    const double needleAngle = 45.0 * M_PI / 180.0;
                    const int needleLength = 20;
                    
                    int startX = cellX - needleLength * cos(needleAngle);
                    int startY = cellY + needleLength * sin(needleAngle);
                    int endX = cellX;
                    int endY = cellY;
