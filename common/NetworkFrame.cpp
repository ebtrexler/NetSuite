...
void NetworkFrame::OnAddCurrent(wxCommandEvent &event)
{
    ...
    if (!selection.IsOk())
    {
        wxMessageBox("Please select a cell or synapse to add current to", "Info", wxOK | wxICON_INFORMATION);
        return;
    }
    ...
    TCurrentUser *currentUser = dynamic_cast<TCurrentUser *>(data->GetObject());
    if (!currentUser)
    {
        wxMessageBox("Please select a cell or synapse to add current to", "Info", wxOK | wxICON_INFORMATION);
        return;
    ...
    wxArrayString types;
    types.Add("TCurrent");
    types.Add("THHCurrent");
    types.Add("TGapJunctionCurrent");
    types.Add("TPlaybackCurrent");
    types.Add("TVoltageClampPIDCurrent");

    CreateItemDialog dlg(this, "Add Current", types);
    if (dlg.ShowModal() == wxID_OK)
    {
        std::wstring type = dlg.GetSelectedType().ToStdWstring();
        std::wstring name = dlg.GetItemName().ToStdWstring();
        
        TCurrentPtr current;
        if (type == L"THHCurrent") {
            current = std::make_shared<THHCurrent>(name, currentUser);
        } else if (type == L"TGapJunctionCurrent") {
            current = std::make_shared<TGapJunctionCurrent>(name, currentUser);
        } else if (type == L"TPlaybackCurrent") {
            current = std::make_shared<TPlaybackCurrent>(name, currentUser);
        } else if (type == L"TVoltageClampPIDCurrent") {
            current = std::make_shared<TVoltageClampPIDCurrent>(name, currentUser);
        } else {
            current = std::make_shared<TCurrent>(name, currentUser);
        }
        
        currentUser->AddCurrent(current);
        UpdateNetworkTree();
        ShowEditPanel(dynamic_cast<TRTBase*>(currentUser)); // Refresh edit panel
        SetStatusText("Current added: " + dlg.GetItemName(), 0);
    }
}
...
void NetworkFrame::ShowEditPanel(TRTBase *obj)
{
    ...
    if (obj)
    {
        ...
        sizer->Add(editPanel, 0, wxEXPAND | wxALL, 5);
...
void NetworkFrame::OnAddButton(wxCommandEvent &event)
{
    ...
    // Check if we're on a category node or an item node
    if (itemText == "Cells" || (data && dynamic_cast<TCell*>(data->GetObject())))
    {
        OnCreateCell(event);
    }
    else if (itemText == "Synapses" || (data && dynamic_cast<TSynapse*>(data->GetObject())))
    {
        OnCreateSynapse(event);
    }
    else if (itemText == "Electrodes" || (data && dynamic_cast<TElectrode*>(data->GetObject())))
    {
...
void NetworkFrame::OnVisualizationSelection(wxCommandEvent &event)
{
    ...
    if (obj)
    {
        ...
        {
            // Refresh visualization panels when position changes
            m_squarePanel->Refresh();
            m_circlePanel->Refresh();
            ShowEditPanel(obj);
        }
